...
//  Limelight
//
//  Created by Bilal Khalid on 2020-01-21.
...
class SwiftPaywall: UIViewController {
    ...
    init(termsOfServiceUrlString: String, // Required terms of service url string
        ...
        edgeStyle: PayWallEdgeStyle = .soft, // Corner radius style, defaults to round
        showDiscountPercentage: Bool = true, // Whether or not to show the discount badge on the products, default to true
        backgroundColor: UIColor = UIColor(gradientStyle:UIGradientStyle.topToBottom, withFrame: System.frame, andColors: BACKGROUNDGRADIENT), // Background color, defults to RevenueCat red
        textColor: UIColor = UIColor.white, // Text color, defaults to white
        productSelectedColor: UIColor = LIMELIGHTPURPLE, // Selected product cell color, defaults to white
        productDeselectedColor: UIColor = UIColor.white) { // Deselected product cell color, defaults to black
    ...
    @objc private func purchaseSelectedPackage() {
        ...
        if indexPath.row == 0 {
            ...
            Subscription.entitlementId = "basic"
             Subscription.isActive = true
            Subscription.periodType = .normal
            Subscription.originalPurchaseDate = 0
             Subscription.latestPurchaseDate = 0
             Subscription.expirationPurchaseDate = 0
            ...
            System.setRevenueCatSubscription(uid: User.uid)
            ...
            Purchases.shared.purchasePackage(package) { (trans, info, error, cancelled) in
                ...
                if let error = error {
                    ...
                    if let purchaseCompletedHandler = self.delegate?.purchaseCompleted {
                        let entitlement = info?.entitlements[0]
                        Subscription.uid = User.uid
    ...
    private func buildSubviews() {
        ...
        headerView = UIView()
        let imageName = "largeLimelightLogo"
           let image = UIImage(named: imageName)
           let imageView = UIImageView(image: image!)
        imageView.frame = CGRect(x: System.frame.midX - 40, y: 60, width: 80, height: 70)
        ...
        titleLabel.font = UIFont(name: "Poppins", size: 26)
        ...
        subtitleLabel.font = UIFont(name: "Poppins", size: 14)
        ...
        buyButton.backgroundColor = LIMELIGHTPURPLE//UIColor(gradientStyle: .leftToRight, withFrame: buyButton.frame, andColors: LIMELIGHTGRADIENT)
        ...
        NSLayoutConstraint.activate([
            ...
            buyButton.bottomAnchor.constraint(equalTo: scrollView.topAnchor, constant: view.frame.height-200), //64
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if #available(iOS 11.2, *) {
            if let introPrice = offering?.availablePackages[indexPath.row].product.introductoryPrice, introPrice.price == 0 {
                ...
                let baseText = NSMutableAttributedString(string: "Includes \(trialLength) free trial. You won't be billed until ")
                let cancelAttributedText = NSAttributedString(string: cancelString, attributes: dateAttributes)
                let and = NSAttributedString(string: " unless you cancel.")
                
                baseText.append(cancelAttributedText)
                baseText.append(and)
                
                if indexPath.row == 0 {
                     subtitleLabel.text = "50KM Starting Radius, 2 Uploads."
                }
                if indexPath.row == 1 {
                     subtitleLabel.text = "100KM Starting Radius, 5 Uploads, Artist Analytics, Artist Verification."
                }
                if indexPath.row == 2 {
                    subtitleLabel.text = "200KM Starting Radius, Unlimited Uploads, Artist Analytics, Artist Verification, Playlist Submission."
                }
...
private class PackageCell : UICollectionViewCell {
    ...
    func setHighlighted( _ highlighted: Bool) {
        if highlighted {
            ...
            durationLabel.textColor = secondaryColor?.withAlphaComponent(1)
            ...
            durationLabel.textColor = highlightColor?.withAlphaComponent(1)
            priceLabel.textColor = highlightColor?.withAlphaComponent(1)
    ...
    func setupWith(
        ...
        edgeStyle: PayWallEdgeStyle = .square,
        ...
        case .custom, .unknown:
            ...
            discountLabel.text = "FREE TRIAL"
            discountLabel.isHidden = false
            monthlyPriceLabel.text = "\(package.localizedPriceString) / mo"
            
            if durationLabel.text == "BASIC" {
                 discountLabel.isHidden = true
                discountLabel.text = "FREE"
                priceLabel.text = "$0"
                 monthlyPriceLabel.text = "Free Forever"
            }
        }
    }
